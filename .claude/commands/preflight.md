...
## Steps
0. **Runtime gate.** Read `.ai-runtime/env-detect.json`. Missing → `NO_ENV_DETECT` (see `/doctor`). `platform_tier == "unsupported"` → `UNSUPPORTED_PLATFORM` (hard STOP); `platform_tier == "best-effort"` (native Windows + Git Bash) → proceed with a warning (no OS-level sandbox; recommend WSL2).
