...
0. **Runtime gate.** Read `.ai-runtime/env-detect.json`. If missing → `NO_ENV_DETECT`. If `platform_tier == "unsupported"` → STOP. If `node_supported == false` → STOP. `best-effort` (native Windows + Git Bash) proceeds, but Docker may be unavailable for the mock image — prefer WSL2/Linux for `/ship-contract`.
