...
`devil` (challenge the design), `auditor` (`/audit` — next-command suggestion from the command log), `brief-synthesizer` (`/synthesize-brief`), `template-sync` (sync a derived project to a newer template version).
