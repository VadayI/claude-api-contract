...
**Version:** 0.2.0 · **Servers:** `https://api.example.com` (Production placeholder — replace in derived projects) · `http://localhost:4010` (Prism mock)
...
| `OAuth2Auth` | oauth2 client-credentials (`tokenUrl: /api/v1/auth/token`, scopes `articles:read`, `articles:write`) | service-flow (S2S, D5); articles (service clients) |
...
|---|---|---|---|---|
| POST | `/api/v1/auth/register` | `registerUser` | public | 201 · 400 · 409 |
| POST | `/api/v1/auth/login` | `loginUser` | public | 200 · 400 · 401 |
| POST | `/api/v1/auth/refresh` | `refreshToken` | public | 200 · 400 · 401 |
| POST | `/api/v1/auth/logout` | `logoutUser` | BearerAuth | 204 · 401 |
| POST | `/api/v1/auth/token` | `issueServiceToken` | public (S2S) | 200 · 400 · 401 |
...
- `/api/v1/auth/token` request body is JSON (not `application/x-www-form-urlencoded`) for a self-contained contract and a trivial mock.
